...
def generate_training_data(df):
    ...
    for _, row in df.iterrows():
        if row['label'] == False:
            cat = {key: False for key in td.CLASSES.values()}
            cat['ABSTAIN'] = True
        else:
            cat = {'ABSTAIN': False}
            for key in td.CLASSES.keys():
                if row[key] != -1:
                    cat[td.CLASSES[key]] = True
                else:
                    cat[td.CLASSES[key]] = False
...
def label_training_data(sections, output_path):
    # output_path = './res/training_data.txt'

    df, res = get_L_train(sections)
    analysis = get_analysis(res)
    # df.query('label == True').to_csv(path_or_buf=output_path)
    rv = generate_training_data(df)
